...
func (k Keeper) BurnTokens(ctx sdk.Context, sender sdk.AccAddress, tokens sdk.Coin) error {
	...
	if err := k.bankKeeper.BurnCoins(
		ctx, types.ModuleName, sdk.NewCoins(tokens)); err != nil {
...
func (k Keeper) SafeMint(ctx sdk.Context, port string, channel string, receiver sdk.AccAddress, denom string, amount int32) error {
	if isIBCToken(denom) {
		...
		if err := k.UnlockTokens(ctx, port, channel, receiver, sdk.NewCoin(denom, sdkmath.NewInt(int64(amount)))); err != nil {
			return err
		}
	}
	return nil
...
func (k Keeper) UnlockTokens(ctx sdk.Context, port string, channel string, receiver sdk.AccAddress, tokens sdk.Coin) error {
	escrowAddress := ibctransfertypes.GetEscrowAddress(port, channel)
